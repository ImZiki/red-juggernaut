<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('banda')" :active="request()->routeIs('banda')">
                        {{ __('Banda') }}
                    </x-nav-link>
                    <x-nav-link :href="route('videos')" :active="request()->routeIs('videos')">
                        {{ __('Videos') }}
                    </x-nav-link>
                    <x-nav-link :href="route('conciertos')" :active="request()->routeIs('conciertos')">
                        {{ __('Conciertos') }}
                    </x-nav-link>
            ...
            <div class="flex items-center space-x-4">
                ...
                <form method="POST" action="{{ route('language.change') }}">
                    @csrf
                    <select name="language" onchange="this.form.submit()" class="bg-transparent border-0 text-gray-800 dark:text-gray-200">
                        <option value="es" {{ session('locale', app()->getLocale()) == 'es' ? 'selected' : '' }}>Español</option>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('banda')" :active="request()->routeIs('banda')">
                {{ __('Banda') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('videos')" :active="request()->routeIs('videos')">
                {{ __('Videos') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('conciertos')" :active="request()->routeIs('conciertos')">
                {{ __('Conciertos') }}
            </x-responsive-nav-link>
